...
export const FightUI: React.FC<FightUIProps> = ({ pokemon, attack, setSelectedMenu }) => {
    ...
    return (
        <Animated.View entering={ZoomIn} className="flex flex-wrap flex-row w-[98%] mt-2 mx-2 ">
            {pokemon.currentMoves.map(move => {
                return (
                    <TouchableOpacity key={move.name} className="w-[48%]" onPress={() => {
                        ...
                        <View style={{ backgroundColor: getBackgroundColour(move.type) }} className="h-8 mx-[2px] 
                        border-white my-[1px] border-2 flex flex-row rounded-md">
