...
export const PokemonShop: React.FC<PokemonShopProps> = ({ setSelectedMenu, team, setTeamIds,
    updateText
}) => {
                    ...
                    return (
                        <TouchableOpacity key={item} onPress={() => setSelectedType(item)}
                            ...
                            <View className={`border-[1px] mx-1 rounded-lg  border-${getBackgroundColour(item)}-500`}>
                                <MyText style="text-white mx-auto text-2xl">{capitalizeFirstLetter(item)} Type</MyText>
                            </View>
                        </TouchableOpacity>
                    )
                })}
