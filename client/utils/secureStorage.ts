...
export const resetPokemon = async () => {
  await SecureStore.setItemAsync("pokemon", "")
}

export const savePokedex = async (pokedex: any) => {
  pokedex.map(p => {
    store.setRow("pokemon", p.name,{name: p.name, image: p.image})
  })
}

export const getStoredPokedex = async () => {
  const originalArray = store.getTable("pokemon")
  const newArray = Object.keys(originalArray).map(key => ({
    name: originalArray[key].name,
    image: originalArray[key].image
  }));
